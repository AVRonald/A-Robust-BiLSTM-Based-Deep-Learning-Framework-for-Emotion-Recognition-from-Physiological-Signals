...
# ---------------------------
# 2. Model - GRUModel (Unidirectional)
# ---------------------------
class GRUModel(nn.Module):
    def __init__(self, input_size, hidden_size=128, num_layers=2, dropout=0.3, output_size=2):
        super().__init__()
        # Unidirectional GRU layers
        self.gru1 = nn.GRU(input_size, hidden_size, num_layers, batch_first=True, bidirectional=False)
        self.bn1 = nn.BatchNorm1d(hidden_size) # No * 2 for unidirectional
        self.gru2 = nn.GRU(hidden_size, hidden_size // 2, num_layers, batch_first=True, bidirectional=False)
        self.bn2 = nn.BatchNorm1d(hidden_size // 2) # No * 2 for unidirectional
        self.gru3 = nn.GRU(hidden_size // 2, hidden_size // 4, num_layers, batch_first=True, bidirectional=False)
        
        self.dropout = nn.Dropout(dropout)
        # Output size from last GRU is (hidden_size // 4) 
    ...
    def forward(self, x):
        ...
        out, _ = self.gru1(x)
        # Permute for BatchNorm1d: (batch, seq_len, hidden_size) -> (batch, hidden_size, seq_len) -> (batch, seq_len, hidden_size)
...
for train_idx, test_idx in skf.split(X_seq, stratify_bins):
    ...
    model = GRUModel(input_size=1, hidden_size=128, num_layers=2, dropout=0.3, output_size=2).to(DEVICE)
