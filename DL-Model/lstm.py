...
class LSTMModel(nn.Module):
    def __init__(self, input_size, hidden_size=128, num_layers=2, dropout=0.3, output_size=2):
        ...
        self.lstm2 = nn.LSTM(hidden_size, hidden_size // 2, num_layers, batch_first=True, bidirectional=False)
        self.bn2 = nn.BatchNorm1d(hidden_size // 2)
        self.lstm3 = nn.LSTM(hidden_size // 2, hidden_size // 4, num_layers, batch_first=True, bidirectional=False)
...
for train_idx, test_idx in skf.split(X_seq, stratify_bins):
    ...
    model = LSTMModel(input_size=1, hidden_size=128, num_layers=2, dropout=0.3, output_size=2).to(DEVICE)
